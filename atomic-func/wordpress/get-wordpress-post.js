...
async function getAllWordpressPost(errorLogPath) {
        ...
        const folder = `${process.cwd()}/options/wordpress/`;
        const cre = readtxt({
        ...
        let postList = await csvFn.readMyCSV({
                headers: ['postID'],
                path: `${getAppRoot()}/options/wordpress/wordpress-post-list.csv`,
        ...
        const promises = postList.map(
                (postID) => limit(
                        () => getOnePost({ page, postID, errorLogPath })
                )
        );
...
async function login({ loginUrl, acc, password, errorLogPath, page }) {
        ...
        await page.$eval("[name=log]", (el, acc) => { el.value = acc }, acc)

        await page.$eval("[name=pwd]", (el, password) => { el.value = password }, password)
...
async function getOnePost({ page, postID, errorLogPath }) {
        ...
        const folder = getAppRoot() + '/options/wordpress/posts';
        ...
        try {
                ...
                const textOfInput = await page.evaluate(() => document.querySelector('[name=post_title]').value);
                fs.writeFileSync(`${folder}/${postID}-title.txt`, helpers.stripWrongHTMLEntities(textOfInput));

                const textOfBody = await page.evaluate(() => document.querySelector("textarea.wp-editor-area").textContent);
                fs.writeFileSync(`${folder}/${postID}-content.txt`, helpers.stripWrongHTMLEntities(textOfBody));
                console.log(`Done saving post - ${textOfInput}`);
        } catch (e) {
                logFn.logError({
                        location: `${process.cwd()}/log/get-posts/${errorLogPath}.txt`,
